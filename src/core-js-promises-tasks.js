...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((settledResults) =>
    settledResults.map((outcomeObject) =>
      outcomeObject.status === 'fulfilled' ? outcomeObject.value : null
    )
  );
}
...
function queuPromises(promises) {
  ...
  promises.forEach((promise) => {
    result = result.then((concatString) =>
      promise.then((value) => concatString + value.toString())
    );
  });
